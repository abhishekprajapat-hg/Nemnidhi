...
const frag = /* glsl */ `
  ...
  const vec3 baseColor1 = vec3(0.404, 0.910, 0.976); // Brand cyan (#67e8f9)
  const vec3 baseColor2 = vec3(0.2, 0.7, 0.85); // Deeper cyan
  const vec3 baseColor3 = vec3(0.05, 0.5, 0.65); // Dark cyan
...
function hexToVec3(color: string) {
  ...
  if (rgbMatch) {
    return new Vec3(Number(rgbMatch[1]) / 255, Number(rgbMatch[2]) / 255, Number(rgbMatch[3]) / 255);
  }

  const hslMatch = color.match(/hsla?\((\d+),\s*(\d+)%,\s*(\d+)%/);
  if (hslMatch) {
    return hslToRgb(Number(hslMatch[1]) / 360, Number(hslMatch[2]) / 100, Number(hslMatch[3]) / 100);
  }
...
export default function Orb({
  ...
  return <div ref={ctnDom} className={styles.orbContainer} aria-hidden="true" />;
}
